...
int displyjpeg(U8 *JPEGFileName,U8 mode,U32 x,U32 y,int member,int denom)
{
	...
	switch(mode)
	{
		...
		case 1:	//在LCD中间显示图片
			...
			{
				//在LCD中间显示图片
				GUI_JPEG_Draw(jpegbuffer,JPEGFile.fsize,(LCD_GetXSize()-XSize)/2-1,(LCD_GetYSize()-YSize)/2-1);
			}else //否则图片需要缩放
			{
...
int displayjpegex(U8 *JPEGFileName,U8 mode,U32 x,U32 y,int member,int denom)
{
	...
	switch(mode)
	{
		...
		case 1:	//在LCD中间显示图片
			...
			{
				//在LCD中间显示图片
				GUI_JPEG_DrawEx(JpegGetData,&JPEGFile,(LCD_GetXSize()-XSize)/2-1,(LCD_GetYSize()-YSize)/2-1);
			}else //否则图片需要缩放
			{
				Xflag = (float)XSize*((float)member/(float)denom);
				Yflag = (float)YSize*((float)member/(float)denom);
...
//		GUI_DispStringHCenterAt("在指定位置显示一张加载到RAM中的JPEG图片",240,0);
		displyjpeg("0:222.jpg",0,0,0,1,1);
		GUI_Delay(100);
		GUI_Clear();
		
		displyjpeg("0:111.jpg",0,0,0,1,1);
		GUI_Delay(100);
		GUI_Clear();
