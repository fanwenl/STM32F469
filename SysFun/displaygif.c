	...
	switch(mode)
	{
		...
		case 1:	//在LCD中间显示图片
			if((member == 1) && (denom == 1)) //无需缩放，直接绘制
			{
				...
				for(i=0;i<GifInfo.NumImages;i++)
				{
					GUI_GIF_DrawSub(gifbuffer,GIFFile.fsize,(LCD_GetXSize()-XSize)/2-1,(LCD_GetYSize()-YSize)/2-1,i);
					GUI_GIF_GetImageInfo(gifbuffer,GIFFile.fsize,&ImageInfo,i);
					GUI_Delay(ImageInfo.Delay ? ImageInfo.Delay*10:100);//延时
				}
			}else //否则图片需要缩放
	...
	switch(mode)
	{
		...
		case 1:	//在LCD中间显示图片
			if((member == 1) && (denom == 1)) //无需缩放，直接绘制
			{
				...
				for(i=0;i<GifInfo.NumImages;i++)
				{
					GUI_GIF_DrawSubEx(GifGetData,&GIFFile,(LCD_GetXSize()-XSize)/2-1,(LCD_GetXSize()-YSize)/2-1,i);
					GUI_GIF_GetImageInfoEx(GifGetData,&GIFFile,&ImageInfo,i);
					GUI_Delay(ImageInfo.Delay ? ImageInfo.Delay*10:100);//延时
				}
			}else //否则图片需要缩放
...
//		GUI_DispStringHCenterAt("在指定位置显示一张缩小1/2的加载到RAM中的GIF图片",400,0);
		displaygif("0:1.gif",1,0,0,1,1);
		GUI_Delay(100);
		GUI_Clear();
		
		displaygif("0:2.gif",1,00,300,1,1);
		GUI_Delay(100);
		GUI_Clear();
		
		displaygif("0:3.gif",1,0,00,1,1);
		GUI_Delay(100);
		GUI_Clear();
