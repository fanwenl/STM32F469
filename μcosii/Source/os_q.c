                  ...
                  INT8U     *perr)
{
    ...
    if (pq->OSQEntries > 0u) {                   /* See if any messages in the queue                   */
        ...
        if (pq->OSQOut == pq->OSQEnd) {          /* Wrap OUT pointer if we are at the end of the queue */
            pq->OSQOut = pq->OSQStart;           /*注意列表的连续性*/
        }
        *perr = OS_ERR_NONE;
    } else {
        *perr = OS_ERR_Q_EMPTY;
    ...
    switch (opt) {
        case OS_DEL_NO_PEND:                               /* Delete queue only if no task waiting     */
             if (tasks_waiting == OS_FALSE) {
                 ...
                 OS_EXIT_CRITICAL();
                 *perr                  = OS_ERR_NONE;     /*可以直接使用 return((OS_EVENT *)0)吗？*/
                ...
                INT8U     *perr)
{
    ...
    OS_ENTER_CRITICAL();
    pq = (OS_Q *)pevent->OSEventPtr;             /* Point at queue control block(在消息队列创建的时候进行关联)*/
    if (pq->OSQEntries > 0u) {                   /* See if any messages in the queue                   */
        pmsg = *pq->OSQOut++;                    /* Yes, extract oldest message from the queue(注意LIFO的模式)*/
        pq->OSQEntries--;                        /* Update the number of entries in the queue          */
        if (pq->OSQOut == pq->OSQEnd) {          /* Wrap OUT pointer if we are at the end of the queue */
                 ...
                 OS_Q_DATA *p_q_data)
{
    ...
    pdest                = &p_q_data->OSEventTbl[0];    /*可以直接使用数组进行复制？*/
    for (i = 0u; i < OS_EVENT_TBL_SIZE; i++) {          /*p_q_data->OSEventTbl[i] = pevent->OSEventTbl[i]*/
        *pdest++ = *psrc++;
    }
    pq = (OS_Q *)pevent->OSEventPtr;
    if (pq->OSQEntries > 0u) {                         /*OSOUT是一个双指针，下面这句话复制的是msg的地址。*/
...
void  OS_QInit (void)
{
    ...
    /*计算一个数组的大小sizeof(OSQTbl)，如果是sizeof(OSQTbl[2])则是计算木一个元素的大小，数组名就代表了数组*/
    OS_MemClr((INT8U *)&OSQTbl[0], sizeof(OSQTbl));  /* Clear the queue table                          */
    for (ix = 0u; ix < (OS_MAX_QS - 1u); ix++) {     /* Init. list of free QUEUE control blocks        */
        ix_next = ix + 1u;                           /*next,OSQTbl[]*/
        pq1 = &OSQTbl[ix];
        pq2 = &OSQTbl[ix_next];
        pq1->OSQPtr = pq2;
    }
    pq1         = &OSQTbl[ix];                      /*最后一个Tbl，QCB*/
    pq1->OSQPtr = (OS_Q *)0;                        /*最后一个QCB指向 0*/
