...
*              Unlike OSMutexPend(), OSMutexAccept() does not suspend(暂停，和延缓) the calling task if the resource is
...
*note：       如果该信号量对本任务后面的代码没有影响则可以使用该函数，当信号量有效时访问信号量，无效是接着
*             执行本任务，如果后面代码需要信号量则需要使得该任务等待
*********************************************************************************************************
*/
...
*                 reduce priority inversion(减少优先级翻转) or 0xFF (OS_PRIO_MUTEX_CEIL_DIS) if priority ceiling
...
#endif
/*如果传递的参数是OS_PRIO_MUTEX_CEIL_DIS则创建的该信号量不进行优先级提升*/
#if OS_ARG_CHK_EN > 0u
    if (prio != OS_PRIO_MUTEX_CEIL_DIS) {                  /*OS_PRIO_MUTEX_CEIL_DIS有可能比OS_LOWEST_PRIO大，先判断*/
...
#endif
    OS_EventWaitListInit(pevent);/*任务等待表中所有任务清0*/
   *perr = OS_ERR_NONE;
    return (pevent);
}
    ...
    switch (opt) {
        case OS_DEL_NO_PEND:                               /* DELETE MUTEX ONLY IF NO TASK WAITING --- */
             if (tasks_waiting == OS_FALSE) {
                 ...
                 if(pcp != OS_PRIO_MUTEX_CEIL_DIS) {
                     OSTCBPrioTbl[pcp] = (OS_TCB *)0;      /* Free up the PCP 释放ＴＣＢ               */
                 }
        ...
        case OS_DEL_ALWAYS:                                /* ALWAYS DELETE THE MUTEX ---------------- */
             ...
             pcp                   = (INT8U)(pevent->OSEventCnt >> 8u);
             if (pcp != OS_PRIO_MUTEX_CEIL_DIS) {
                 OSTCBPrioTbl[pcp] = (OS_TCB *)0;          /* Free up the PCP释放TCB*/
             }
             pevent->OSEventType   = OS_EVENT_TYPE_UNUSED; /*标记为信号量未使用*/
             pevent->OSEventPtr    = OSEventFreeList;      /* Return Event Control Block to free list  */
             pevent->OSEventCnt    = 0u;
...
{
    INT8U      pcp;                                        /* Priority Ceiling Priority (PCP)          */
    INT8U      mprio;                                      /* Mutex owner priority(占用信号量的任务)   */
    BOOLEAN    rdy;                                        /* Flag indicating task was ready           */
    OS_TCB    *ptcb;
    OS_EVENT  *pevent2;                                   /*为什么要申明这样一个变量？*/
    INT8U      y;
#if OS_CRITICAL_METHOD == 3u                               /* Allocate storage for CPU status register */
    OS_CPU_SR  cpu_sr = 0u;
#endif
...
/*$PAGE*/
    OS_ENTER_CRITICAL();
    pcp = (INT8U)(pevent->OSEventCnt >> 8u);               /* Get PCP from mutex                       */
        /* Is Mutex available? 如果信号量可以用，直接返回函数，可以去访问信号量了                     */
    if ((INT8U)(pevent->OSEventCnt & OS_MUTEX_KEEP_LOWER_8) == OS_MUTEX_AVAILABLE) {
        pevent->OSEventCnt &= OS_MUTEX_KEEP_UPPER_8;       /* Yes, Acquire the resource                */
  /*OSEventCnt和OSEventPtr储存的是当前占有信号量的任务，OSMutexPost()函数不可能占有信号量，只有OSMutexPend()能占有*/
    ...
    /*信号量不可用，如果任务等级可以提升那么将占用信号量的任务提升，否则任务将等待*/
    if (pcp != OS_PRIO_MUTEX_CEIL_DIS) {
/*
********************************************************************************************************
*mprio(在信号量发送函数中设置)和pevent->OSEventPtr是信号占用者的信息，没有释放型号量,也就是另一个任务中占
*有该信号量，在另一个任务的OSMutexPend()函数中资源没有释放，设置为该任务的优先级和任务指针.
*********************************************************************************************************
*/      
        ...
        if (ptcb->OSTCBPrio > pcp) {                              /*     Need to promote prio of owner?*/
            if (mprio > OSTCBCur->OSTCBPrio) {                    /*优先级(小的高)：pcp高于占有信号量的优先级*/
                y = ptcb->OSTCBY;                                 /*当前任务的优先级高于有信号量的优先级*/
/*优先级A>b>c,C没有释放信号量，B剥夺了C，A要使用信号量，所以需要提升C的优先级，检查C是不是就绪(任务被挂起，或者休眠)*/
                if ((OSRdyTbl[y] & ptcb->OSTCBBitX) != 0u) {      /*     See if mutex owner is ready   */
                    ...
                    }                                             /*注意理解任务的就绪状态，当任务被创建好之后*/
                    rdy = OS_TRUE;                                /*任务就进入等待状态，就绪状态可以随时执行*/
                } else {                                          /*相对于等待状态，和休眠状态，       */
                    pevent2 = ptcb->OSTCBEventPtr;                /*挂起是中断状态(调度)               */
    ...
    switch (OSTCBCur->OSTCBStatPend) {                /* See if we timed-out or aborted                */
        ...
        default:
             OS_EventTaskRemove(OSTCBCur, pevent);    /*超时，从任务等待表中删除*/
             *perr = OS_ERR_TIMEOUT;                  /* Indicate that we didn't get mutex within TO   */
             break;
    }
    OSTCBCur->OSTCBStat          =  OS_STAT_RDY;      /* Set   task  status to ready                   */
    OSTCBCur->OSTCBStatPend      =  OS_STAT_PEND_OK;  /* Clear pend  status                            */
    /*在任务等待函数OS_EventTaskWait()中对OSTCBEventPtr赋值，记录等待任务要访问的事件*/
    /*还在OS_EventTaskRdy()中赋值，用于记录multi-pend event多事件状态 os_core 1111行*/
...
INT8U  OSMutexPost (OS_EVENT *pevent)
{
    ...
    if (pevent->OSEventGrp != 0u) {                   /* Any task waiting for the mutex?               */
        ...
        pevent->OSEventCnt |= prio;                   /*一个ECB可能被多个TCB任务使用，需要找到优先级最高的任务*/
                     ...
                     OS_MUTEX_DATA  *p_mutex_data)
{
    ...
    if (p_mutex_data->OSOwnerPrio == 0xFFu) {
        p_mutex_data->OSValue = OS_TRUE;                  /*信号量可用*/
    } else {
...
*
*              prio            is the desired priority(原来的优先级，提升的优先级在TCB中)
*
* Returns    : none
*********************************************************************************************************
*/
                                 ...
                                 INT8U    prio)
{
    ...
    OSRdyTbl[y] &= (OS_PRIO)~ptcb->OSTCBBitX;              /*在任务就绪表中移除原来的优先级就绪状态*/
    if (OSRdyTbl[y] == 0u) {
        OSRdyGrp &= (OS_PRIO)~ptcb->OSTCBBitY;
    }
    ptcb->OSTCBPrio         = prio;
    OSPrioCur               = prio;                        /* The current task is now at this priority */
#if OS_LOWEST_PRIO <= 63u                                  /*计算优先级分组*/
