...
*              priority MUST be available.(注意优先级必须可用)
*
...
/*$PAGE*/
    ...
    if (OSTCBPrioTbl[newprio] != (OS_TCB *)0) {             /* New priority must not already exist     */
      ...
      *不太明白为啥自己的优先级是FF，需要用户在自己的软件中设置，如果在该任务中需要改变该任务的优先级，
    那么在调用该函数时，用户需要将优先级设置为FF(也就是OS_PRIO_SELF)*/
    ...
    /*如果ptcb的指针是OS_TCB_RESERVED说明这个任务已经被保留了，在任务创建函数中有保护(本文245行)*/
    ...
    if ((OSRdyTbl[y_old] &   bitx_old) != 0u) {             /* If task is ready make it not            */
         OSRdyTbl[y_old] &= (OS_PRIO)~bitx_old;             /*如果该任务准备就绪，将该任务注销*/
         if (OSRdyTbl[y_old] == 0u) {                       /*如果该组中的所有任务均没有就绪，将该组清0*/
    ...
    if (ptcb->OSTCBEventMultiPtr != (OS_EVENT **)0) {
        pevents =  ptcb->OSTCBEventMultiPtr;                    /*双指针变量，OSTCBEv...Mu..中存的是一个指针*/
        pevent  = *pevents;                                     /*将pevents的内容(是个指针)付给pevent*/                                                        
                     ...
                     INT8U    prio)
{
    ...
    if (OSIntNesting > 0u) {                 /* Make sure we don't create the task from within an ISR  */
        OS_EXIT_CRITICAL();                  /*OSIntNesting 全局变量，没进入一次中断，该变量+1         */
        return (OS_ERR_TASK_CREATE_ISR);
    }   
    /*OSTCBPrioTbl[prio]是指向任务控制块的指针，每个元素都是指向OSTCB的指针，prio是任务优先级,如果为空该优先级没有使用*/
    if (OSTCBPrioTbl[prio] == (OS_TCB *)0) { /* Make sure task doesn't already exist at this priority  */
        ...
        if (err == OS_ERR_NONE) {
            if (OSRunning == OS_TRUE) {      /* Find highest priority task if multitasking has started */
                OS_Sched();                  /*任务调度是否有新的优先级高的任务准备就绪*/
            }
        } else {
            OS_ENTER_CRITICAL();             /*OS_TCBInit()初始化失败*/
...
#if (OS_TASK_STAT_STK_CHK_EN > 0u)                              /*堆栈检查开关                        */
...
*                 (看邵贝贝书中将的，将调度器锁定，删除任务分两次进行，在响应完中断之后，会进行调度，回到当前任务)
*********************************************************************************************************
*/
...
/*$PAGE*/
    ...
    if (ptcb->OSTCBPrev == (OS_TCB *)0) {               /* Remove from TCB chain                       */
        ptcb->OSTCBNext->OSTCBPrev = (OS_TCB *)0;       /*如果在链表开始*/
        OSTCBList                  = ptcb->OSTCBNext;
    } else {
        ptcb->OSTCBPrev->OSTCBNext = ptcb->OSTCBNext;   /*在链表的中间*/
...
*                   a) notify a task to delete itself.(通知任务删除自己)
...
#if OS_ARG_CHK_EN > 0u
    if (prio >= OS_LOWEST_PRIO) {                               /* Task priority valid ?               */
        if (prio != OS_PRIO_SELF) {/*应为自己的优先级的是FF，所以当大于最低的优先级之后，还需要判断是不是自己*/
            return (OS_ERR_PRIO_INVALID);
        }
    }
#endif
                      ...
                      INT8U   *perr)
{
    ...
    *pname = ptcb->OSTCBTaskName;
    len    = OS_StrLen(*pname);    /*计算Pname的长度*/
    OS_EXIT_CRITICAL();
    *perr  = OS_ERR_NONE;
    return (len);
}
                     ...
                     OS_STK_DATA  *p_stk_data)
{
    OS_TCB    *ptcb;
    OS_STK    *pchk;
    INT32U     nfree;                                  /*一般是OSTaskCreateExt()使用吧*/
    INT32U     size;
#if OS_CRITICAL_METHOD == 3u                           /* Allocate storage for CPU status register     */
    OS_CPU_SR  cpu_sr = 0u;
#endif
    ...
    OS_EXIT_CRITICAL();
#if OS_STK_GROWTH == 1u                               /*在这里注意理解栈顶和栈底*/
...
*              an event (i.e. a message, a semaphore, a queue ...) you will prevent(阻止) this task from
...
INT8U  OSTaskSuspend (INT8U prio)
{
    ...
    if (prio == OS_PRIO_SELF) {                                 /* See if suspend SELF                 */
        prio = OSTCBCur->OSTCBPrio;
        self = OS_TRUE;                                         /*看这里写法，可以在用户程序中指定是不是自己*/
                    ...
                    OS_TCB  *p_task_data)
{
    ...
    if (ptcb == OS_TCB_RESERVED) {               /* Task to query must not be assigned to a Mutex      */
      ...
      /*内容的复制，直接用指针操作*/             /* Copy TCB into user storage area                    */
                      ...
                      INT8U  *perr)
{
    ...
    value = ptcb->OSTCBRegTbl[id];              /*需要预先的存储，在OSTCB结构体中，也可以使用OSTaskCreateExt()预习写入*/
    OS_EXIT_CRITICAL();
    *perr = OS_ERR_NONE;
    return (value);
}
...
INT8U  OSTaskRegGetID (INT8U  *perr)
{
    ...
    /*动态分配ID，每一个任务控制块可以有多个ID寄存器，在程序中有时可能会需要将OSTaskRegNextAvailID清零，如果是在不同的任务中*/
                    ...
                    INT8U   *perr)
{
    ...
    ptcb->OSTCBRegTbl[id] = value;             /*将值写入对应的ID中*/
    OS_EXIT_CRITICAL();
    *perr                 = OS_ERR_NONE;
}
#endif
...
*
* Description: This function is called if a task accidentally(意外的) returns without deleting itself.  In other
