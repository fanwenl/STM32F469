                        ...
                        INT8U        *perr)
{
    ...
    result = (INT8U)(wait_type & OS_FLAG_CONSUME);         /*判断是不是使用consume，在wait_type参数传入的时候设定*/
    if (result != (INT8U)0) {                              /* See if we need to consume the flags      */
        wait_type &= ~OS_FLAG_CONSUME;                     /*取消consume的标志位，将consume设置为true*/
        consume    = OS_TRUE;
    } else {
        consume    = OS_FALSE;
    }
    ...
    *perr = OS_ERR_NONE;                                   /* Assume NO error until proven otherwise.  */
    ...
    switch (wait_type) {
        case OS_FLAG_WAIT_SET_ALL:                         /* See if all required flags are set        */
             ...
             } else {                                      /*不论信号量是不是有效，均执行后面的程序(任务)*/
                 *perr = OS_ERR_FLAG_NOT_RDY;              /*主要是在err上区分，不将任务进入等待状态*/
             }
             OS_EXIT_CRITICAL();
             break;
...
#if OS_FLAG_WAIT_CLR_EN > 0u                              /*有效位为0*/
...
OS_FLAG_GRP  *OSFlagCreate (OS_FLAGS  flags,         /*事件标志组的初始值*/
                            INT8U    *perr)
{
    OS_FLAG_GRP *pgrp;
#if OS_CRITICAL_METHOD == 3u                        /* Allocate storage for CPU status register        */
                         ...
                         INT8U        *perr)
{
    ...
    OS_ENTER_CRITICAL();
    /*如果OSFlagWaitList不等于0说明有node，有等待任务*/
...
#if OS_FLAG_NAME_EN > 0u
                 pgrp->OSFlagName     = (INT8U *)(void *)"?";
#endif           /*将信号量标志组从链表中删除，返回到空闲标志链表中*/
                      ...
                      INT8U        *perr)
{
    ...
    result = (INT8U)(wait_type & OS_FLAG_CONSUME);         /*是否需要consume*/
    if (result != (INT8U)0) {                              /* See if we need to consume the flags      */
...
/*$PAGE*/
    OS_ENTER_CRITICAL();
    switch (wait_type) {/*OSFlagFlags当前发生的信号量，flags想要的信号量，均为1*/
        case OS_FLAG_WAIT_SET_ALL:                         /* See if all required flags are set        */
             flags_rdy = (OS_FLAGS)(pgrp->OSFlagFlags & flags);   /* Extract only the bits we want     */
             if (flags_rdy == flags) {                     /* Must match ALL the bits that we want     */
                 if (consume == OS_TRUE) {                 /* See if we need to consume the flags      */
                 ...
                 *perr                   = OS_ERR_NONE;
                 ...
                 OS_FlagBlock(pgrp, &node, flags, wait_type, timeout);/*任务等待*/
                 OS_EXIT_CRITICAL();
             }
             break;
        ...
        case OS_FLAG_WAIT_SET_ANY:
             ...
             break;
                                                  /*在flags中要求的事件没有发生，则继续执行任务，发生则等待*/
#if OS_FLAG_WAIT_CLR_EN > 0u                      /*是否所有请求的标志位都被置0，要求这些事件都不发生！*/
        case OS_FLAG_WAIT_CLR_ALL:                         /* See if all required flags are cleared    */
          /*想要的位在flags中为1，在OSFlagsFlags中为0*/   
          flags_rdy = (OS_FLAGS)~pgrp->OSFlagFlags & flags;    /* Extract only the bits we want     */
             if (flags_rdy == flags) {                     /* Must match ALL the bits that we want     */
                 if (consume == OS_TRUE) {                 /* See if we need to consume the flags      */
                     pgrp->OSFlagFlags |= flags_rdy;       /* Set ONLY the flags that we wanted        */
...
/*$PAGE*/
    /*没有return，没有满足想要位的条件，任务被等待进行调度，再次回到该任务（超时或者是信号量被释放）*/
    OS_Sched();                                            /* Find next HPT ready to run               */
    OS_ENTER_CRITICAL();/*等待超时或者是被取消*/
    if (OSTCBCur->OSTCBStatPend != OS_STAT_PEND_OK) {      /* Have we timed-out or aborted?            */
        pend_stat                = OSTCBCur->OSTCBStatPend;
        OSTCBCur->OSTCBStatPend  = OS_STAT_PEND_OK;
        OS_FlagUnlink(&node);                              /*将node从链表中删除*/
    ...
    flags_rdy = OSTCBCur->OSTCBFlagsRdy;                  /*清除已经使用（访问）的信号量集*/
    if (consume == OS_TRUE) {                              /* See if we need to consume the flags      */
        switch (wait_type) {
...
*              set or clear are specified by a 'bit mask'.(清除或者置位flag)
*              发送信号量，对应信号量集标志组OSFlagFlags中的位置1或者清0，，表示该任务释放了，信号量，其他的
*              任务可以去访问了，和OSFlagNodeFlags(需要的信号量)进行witeType运算，满足预设条件，则任务恢复运行。
* Arguments  : pgrp          is a pointer to the desired event flag group.
*
...
/*$PAGE*/
    ...
    pnode = (OS_FLAG_NODE *)pgrp->OSFlagWaitList;    /*下面语句遍历所有的节点，将满足的任务都就绪*/
    while (pnode != (OS_FLAG_NODE *)0) {             /* Go through all tasks waiting on event flag(s)  */
        switch (pnode->OSFlagNodeWaitType) {
            case OS_FLAG_WAIT_SET_ALL:               /* See if all req. flags are set for current node */
                 flags_rdy = (OS_FLAGS)(pgrp->OSFlagFlags & pnode->OSFlagNodeFlags);
                 /*判断是不是全部的任务都发生了，是不是满足OSFlagNodeFlags这个条件(等待任务的条件)，满足则将该任务就绪*/
...
*              event flag bit(s) are set.(阻塞任务，直到需要的信号量被set)
*
* Arguments  : pgrp          is a pointer to the desired event flag group.
*
                            ...
                            INT32U        timeout)
{
    ...
    pnode->OSFlagNodePrev     = (void *)0;            /*在事件标志等待列表的开始增加node。*/
    pnode->OSFlagNodeFlagGrp  = (void *)pgrp;         /* Link to Event Flag Group                      */
    pnode_next                = (OS_FLAG_NODE *)pgrp->OSFlagWaitList;
    if (pnode_next != (void *)0) {                    /* Is this the first NODE to insert?             */
        pnode_next->OSFlagNodePrev = pnode;           /* No, link in doubly linked list                */
    }/*注意理解这部分链表的添加，OSFlagWaitList在创建的时候赋值为0，如果原来已经有了pnode这需要将next的pnode指向新增加的node*/
    pgrp->OSFlagWaitList = (void *)pnode;/*不论原来是不是已经有pnode，都要执行这句话，将OSFlagWaitList指向pnode*/

    y            =  OSTCBCur->OSTCBY;                 /* Suspend current task until flag(s) received   */
    OSRdyTbl[y] &= (OS_PRIO)~OSTCBCur->OSTCBBitX;     /*阻塞任务，找到flag被接收*/
    if (OSRdyTbl[y] == 0x00u) {
...
{
#if OS_MAX_FLAGS == 1u                                              /*OSFlagTbl[0]数组在ucosii.h中定义*/
...
#endif
    }/*链表的尾部，(最后一个是一个特例，需要从上面的循环中拿出来)for循环中是“<”不是“<=”*/
    pgrp1                 = &OSFlagTbl[ix];
    pgrp1->OSFlagType     = OS_EVENT_TYPE_UNUSED;
    ...
    pgrp1->OSFlagName     = (INT8U *)(void *)"?";                   /* Unknown name                    */
#endif
    OSFlagFreeList        = &OSFlagTbl[0];                          /*初始化完成指向表头*/
#endif
}
...
*              desired event flag bits have been set.(获得了flag 标志)
*
                                 ...
                                 INT8U         pend_stat)
{
    ...
    ptcb->OSTCBStat     &= (INT8U)~(INT8U)OS_STAT_FLAG;    /*清除Flag等待标志位*/
    ptcb->OSTCBStatPend  = pend_stat;                      
    if (ptcb->OSTCBStat == OS_STAT_RDY) {                  /* Task now ready? 是否有其他事件等待？*/
