                      ...
                      INT8U  *perr)
{
    ...
    OS_CPU_SR  cpu_sr = 0u;                           /*初始化cpu_sr变量为后续开、关中断做准备*/
#endif
...
#if OS_ARG_CHK_EN > 0u                                /*参数检查,判断的是该地址（addr）是否合法，不是其中的内容*/
    if (addr == (void *)0) {                          /* Must pass a valid address for the memory part.*/
        *perr = OS_ERR_MEM_INVALID_ADDR;
        return ((OS_MEM *)0);
    }                                                   /*sizeof(void *)获得指针在内存中占的字节，如果占1个Uint*/
                                                        /*减完之后为0，addr也是Uint，&之后为0，则指针对齐，    */
...
#endif
    OS_ENTER_CRITICAL();                              /*屏蔽中断*/
    /*OSMenFreeList 是一个外部变量在ucos_ii.h中申明，在OS_MemInit()中指向空的内存控制块*/
    ...
    pblk  = (INT8U *)addr;
    loops  = nblks - 1u;
    for (i = 0u; i < loops; i++) {                    /*认真思考，plink是指向下一个内存块，如果要讲pblk的*/
                                                      /*值（下一个内存块）存到plink中，则使用*plink = pblk*/
                                                      /*如果要方位pblk中的内容，就要用到**plink          */
                     ...
                     INT8U    *perr)
{
    ...
    OS_ENTER_CRITICAL();
    *pname = pmem->OSMemName;                    /*OsMemName中存放的是一个指针，要访问其中的内容，要定义*/
                                                 /*双指针，这句话的意思讲OSMEMName中存的地址付给*pname，*/
                                                 /***pname中就是Name的名字                              */
    len    = OS_StrLen(*pname);
    OS_EXIT_CRITICAL();
    *perr  = OS_ERR_NONE;
                 ...
                 void    *pblk)
{
    ...
    *(void **)pblk      = pmem->OSMemFreeList;   /* Insert released block into free block list         */
    pmem->OSMemFreeList = pblk;                  /*首先将OSMenFreeList的内容给了*pblk,然后将Pblk给FreeList*/
...
{
#if OS_MAX_MEM_PART == 1u                                 /*只是初始化内存控制块链表*/
    OS_MemClr((INT8U *)&OSMemTbl[0], sizeof(OSMemTbl));   /* Clear the memory partition table          */
    OSMemFreeList               = (OS_MEM *)&OSMemTbl[0]; /* Point to beginning of free list(链表跟指针)           */
#if OS_MEM_NAME_EN > 0u                                   /*只有一个链表FreeList指向该链表*/
    ...
    pmem                = &OSMemTbl[i];
    pmem->OSMemFreeList = (void *)0;                      /* Initialize last node(节点)                      */
#if OS_MEM_NAME_EN > 0u
    pmem->OSMemName = (INT8U *)(void *)"?";
#endif
