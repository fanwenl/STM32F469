                     ...
                     INT8U     *perr)
{
    ...
    switch (opt) {
        case OS_DEL_NO_PEND:                               /* Delete semaphore only if no task waiting */
             ...
             break;
   /*信号量强制删除，将信号量添加到链表中。如果任务在等待执行一次调度，没有等待返回删除成功*/
...
/*信号量不可用，使任务进入等待状态，在等待状态只有OSSemPost()函数释放型号量，或者是等待超时，才能被重新调度*/
        ...
        switch (opt) {
            case OS_PEND_OPT_BROADCAST:               /* Do we need to abort ALL waiting tasks?        */
                 while (pevent->OSEventGrp != 0u) {   /* Yes, ready ALL tasks waiting on semaphore     */
                     (void)OS_EventTaskRdy(pevent, (void *)0, OS_STAT_SEM, OS_STAT_PEND_ABORT);
                     nbr_tasks++;
                 }/*每调用一次将最高优先级的任务就绪，直到OSEventGrp为0 没有等待任务。*/
                 break;
...
INT8U  OSSemPost (OS_EVENT *pevent)
{
    ...
    if (pevent->OSEventGrp != 0u) {                   /* See if any task waiting for semaphore         */
        /*有任务等待*/                                              /* Ready HPT waiting on event                    */
                   ...
                   OS_SEM_DATA  *p_sem_data)
{
    ...
    OS_ENTER_CRITICAL();
    p_sem_data->OSEventGrp = pevent->OSEventGrp;           /* Copy message semaphore wait list           */
    psrc                   = &pevent->OSEventTbl[0];
    pdest                  = &p_sem_data->OSEventTbl[0];/*获得两个数组的开始地址，一一复制*/
                ...
                INT8U     *perr)
{
    ...
    OS_ENTER_CRITICAL();
    *perr = OS_ERR_NONE;
    if (pevent->OSEventCnt > 0u) {                    /* See if seaphore already has a count          */
