                       ...
                       INT8U      *perr)
{
    ...
    *pname = pevent->OSEventName;
    len    = OS_StrLen(*pname);               /*计算name的值，将此值返回该函数*/
    OS_EXIT_CRITICAL();
    *perr  = OS_ERR_NONE;
    return (len);
}
                      ...
                      INT8U     *perr)
{
    ...
    switch (pevent->OSEventType) {              /*检查事件的类型*/
        case OS_EVENT_TYPE_SEM:
        case OS_EVENT_TYPE_MUTEX:
        case OS_EVENT_TYPE_MBOX:
        case OS_EVENT_TYPE_Q:
    ...
    OS_ENTER_CRITICAL();
    pevent->OSEventName = pname;                /*将ASCII字符赋值到OSEventName中*/
    OS_EXIT_CRITICAL();
    *perr = OS_ERR_NONE;
}
#endif
            ...
            case OS_EVENT_TYPE_MBOX:
                 if (pevent->OSEventPtr != (void *)0) { /* If mailbox NOT empty;                   ... */
                    ...
                    *pmsgs_rdy++         = (void *)pevent->OSEventPtr;
                     pevent->OSEventPtr  = (void *)0;   /*把消息取走之后，将这个清除*/
...
*                                              ENTER ISR
*
* Description: This function is used to notify(通知) uC/OS-II that you are about to service an interrupt
*              service routine (ISR服务程序).  This allows uC/OS-II to keep track of interrupt nesting and thus
...
* Returns    : none
*
* Notes      : 1) This function should be called with interrupts already disabled(中断禁止后调用)
*              2) Your ISR can directly increment OSIntNesting without calling this function because
*                 OSIntNesting has been declared 'global'.
*              3) You MUST still call OSIntExit() even though you increment OSIntNesting directly(直接的).
...
void  OSIntEnter (void)
{
    if (OSRunning == OS_TRUE) {
        if (OSIntNesting < 255u) {                      
            OSIntNesting++;                      /* Increment ISR nesting level(增加中断的嵌套层数)      */
        }
    }
}
/*$PAGE*/
    ...
    if (OSRunning == OS_TRUE) {
        ...
        if (OSIntNesting == 0u) {                          /* Reschedule only if all ISRs complete ... */
            if (OSLockNesting == 0u) {                     /* ... and not locked.                      */
                OS_SchedNew();                             /*查找就绪优先级最高的任务，                */
                OSTCBHighRdy = OSTCBPrioTbl[OSPrioHighRdy];/*得到优先级最高的任务控制块*/
                if (OSPrioHighRdy != OSPrioCur) {          /* No Ctx Sw if current task is highest rdy */
#if OS_TASK_PROFILE_EN > 0u                                /*增加该任务转换次数*/
        ...
        if (OSIntNesting == 0u) {                          /* Can't call from an ISR                   */
            if (OSLockNesting > 0u) {                      /* Do not decrement if already 0            */
                ...
                if (OSLockNesting == 0u) {                 /* See if scheduler is enabled              */
                    OS_EXIT_CRITICAL();                    /*如果OSLockNesting==0 则执行一次调度，否则直接返回*/
...
*
* Description: This function is called by your application to establish(建立) CPU usage by first determining
...
{
    OS_TCB    *ptcb;
#if OS_TICK_STEP_EN > 0u                                    /* uC/OS-Viewh插件的使用                  */
    BOOLEAN    step;
#endif
#if OS_CRITICAL_METHOD == 3u                               /* Allocate storage for CPU status register     */
    OS_CPU_SR  cpu_sr = 0u;
...
#endif
#if OS_TIME_GET_SET_EN > 0u                                /* Include code for OSTimeGet() and OSTimeSet()             */
...
#endif  /*遍历所有的控制块，将OSTCBDly减一，使没挂起的任务进入就绪状态*/
        ptcb = OSTCBList;                                  /* Point at first TCB in TCB list               */
        while (ptcb->OSTCBPrio != OS_TASK_IDLE_PRIO) {     /* Go through all TCBs in TCB list              */
            OS_ENTER_CRITICAL();
            if (ptcb->OSTCBDly != 0u) {                    /* No, Delayed or waiting for event with TO     */
                ptcb->OSTCBDly--;                          /* Decrement nbr of ticks to end of delay       */
                if (ptcb->OSTCBDly == 0u) {                /* Check for timeout(超时)                     */
                     /*有状态标志有没有事件等待，不管挂起的标志位(OS_STAT_SUSPEND)*/
                    ...
                    /*如果没有Ssuspended(挂起)，则将任务至于就绪状态 ，注意在任务中 任务的挂起和等待是两个概念*/
                    /*挂起的任务有OSTaskReume()来恢复*/
                        ...
                        INT8U      pend_stat)
{
    ...
    /*->的优先级高于&的优先级，取得OSEventTbl[y]的地址。哈哈*/
   ...
   /* See if task is ready (could be susp'd)    任务没有被暂停  */
...
void  OS_EventTaskWaitMulti (OS_EVENT **pevents_wait)
{
    ...
    OSTCBCur->OSTCBEventPtr      = (OS_EVENT  *)0;                  /*将EventPtr清零，将EventMultiPtr指向多重事件列表*/
                               ...
                               OS_EVENT **pevents_multi)
{
    ...
    pevent  = *pevents;
    /*将多重任务中的每个事件一一从任务等待表中删除，任务等待表中的标志和就绪表中的标志一一对应的关系*/
...
#endif                                                     /*调用函数OSStatInit()之后变量为OS_TRUE*/
...
static  void  OS_SchedNew (void)
{
    ...
    y             = OSUnMapTbl[OSRdyGrp];        /*在OSUnMapTbl中查找*/
    ...
    if ((OSRdyGrp & 0xFFu) != 0u) {
        y = OSUnMapTbl[OSRdyGrp & 0xFFu];       /*如果低8位中有不为0的位数，则最高优先级的任务肯定在低8位中*/
    } else {
      /*如果最高优先级的位数在高8位中，先将高八位右移8位，查表，然后再加8.应为右移了8位所以要加**/
...
void  OS_TaskStat (void *p_arg)
{
    ...
    p_arg = p_arg;                               /* Prevent compiler warning for not using 'p_arg'     */
    while (OSStatRdy == OS_FALSE) {              /*统计任务没有初始化完成，等待*/
        OSTimeDly(2u * OS_TICKS_PER_SEC / 10u);  /* Wait until statistic task is ready                 */
    }
    OSIdleCtrMax /= 100uL;                      /*根据CPU使用率的公式计算，如果这个值除以100以后小于0，则系统不稳定，将自己挂起*/
    if (OSIdleCtrMax == 0uL) {                  /*将公式简化之后为100-OSIdleCtr/(OSIdleCtrMax/100),OSIdleCtr是一个整数，除数不能为0*/
        OSCPUUsage = 0u;
#if OS_TASK_SUSPEND_EN > 0u
        (void)OSTaskSuspend(OS_PRIO_SELF);      /*将任务挂起，需要使用需要OSTaskResume()来恢复任务*/
#else
        for (;;) {
    ...
    OS_ENTER_CRITICAL();
    OSIdleCtr = OSIdleCtrMax * 100uL;            /* Set initial CPU usage as 0%(初始值)*/
    OS_EXIT_CRITICAL();
    for (;;) {
        OS_ENTER_CRITICAL();
        ...
        OS_EXIT_CRITICAL();
        OSCPUUsage   = (INT8U)(100uL - OSIdleCtrRun / OSIdleCtrMax);/*OSIdleCtrMax在函数开始已经除以100了*/
        OSTaskStatHook();                        /* Invoke user definable hook                         */
#if (OS_TASK_STAT_STK_CHK_EN > 0u) && (OS_TASK_CREATE_EXT_EN > 0u)
        OS_TaskStatStkChk();                     /* Check the stacks for each task                     */
#endif
        OSTimeDly(OS_TICKS_PER_SEC / 10u);       /* Accumulate(累积) OSIdleCtr for the next 1/10 second      */
    }
}
#endif
/*$PAGE*/
...
void  OS_TaskStatStkChk (void)
{
    ...
    for (prio = 0u; prio <= OS_TASK_IDLE_PRIO; prio++) {             /*遍历所有的优先级*/
        err = OSTaskStkChk(prio, &stk_data);
        if (err == OS_ERR_NONE) {                                    /*堆栈没有错误*/
            ptcb = OSTCBPrioTbl[prio];
            if (ptcb != (OS_TCB *)0) {                               /* Make sure task 'ptcb' is ...   */
                if (ptcb != OS_TCB_RESERVED) {                       /* ... still valid.               */
#if OS_TASK_PROFILE_EN > 0u
                    #if OS_STK_GROWTH == 1u                           /*STCBStkBase栈顶指针*/
                    ptcb->OSTCBStkBase = ptcb->OSTCBStkBottom + ptcb->OSTCBStkSize;
                    #else
                    ptcb->OSTCBStkBase = ptcb->OSTCBStkBottom - ptcb->OSTCBStkSize;
                    #endif
                    ptcb->OSTCBStkUsed = stk_data.OSUsed;            /* Store number of entries used   */
#endif      /*这个函数检测每个任务的堆栈使用情况，通过调用参数OSTCBStkBase，OSTCBStkUsed可以知道TCB中堆栈的使用*/
                }
            }
        }
    }
...
*                            (额外的堆栈指针，可以存储FPU registers ，MMU 。。。OSTaskCreateExt()使用)
*              opt           options as passed to 'OSTaskCreateExt()' or,
*                            0 if called from 'OSTaskCreate()'.
*                            (opt在OSTaskCreatExt()中可用于是否使用浮点寄存器 在os_cpu.c中有体现)
...
/*初始化OSTCBDly这个变量默认是0，在任务调用OSTimeDly()函数时，将延时值赋给OSTCBDly中。在OSTimeTick中减少*/                               
...
#if OS_LOWEST_PRIO <= 63u  /*在任务就绪表中存储标志位*/            /* Pre-compute X, Y                  */
        ptcb->OSTCBY             = (INT8U)(prio >> 3u);            /*最大63个任务，高3位做为OSRdyGrp，低3位做为OSRdyTbl*/
        ptcb->OSTCBX             = (INT8U)(prio & 0x07u);         /*00111111,64个任务8个table，每个table8位        */
#else                                                              /*256个任务  Pre-compute X, Y                  */
        ptcb->OSTCBY             = (INT8U)((INT8U)(prio >> 4u) & 0xFFu);
        ptcb->OSTCBX             = (INT8U) (prio & 0x0Fu);
#endif
                                                                  /* Pre-compute BitX and BitY         */
        ptcb->OSTCBBitY          = (OS_PRIO)(1uL << ptcb->OSTCBY);/*将对应的位置1，假如OSTCBY=0x03则移位后*/
        ptcb->OSTCBBitX          = (OS_PRIO)(1uL << ptcb->OSTCBX);/*BitY=00001000,此值对应于OSRdyGrp*/
        /*OSTCBBitX对应于OSRdyTbl[]中的位数*/
#if (OS_EVENT_EN)
        ...
        ptcb->OSTCBPrev = (OS_TCB *)0;                     /*插在链表的头*/
        if (OSTCBList != (OS_TCB *)0) {
            OSTCBList->OSTCBPrev = ptcb;
        }
        OSTCBList               = ptcb;                    /*OSTCBList 指向链表的开始*/
