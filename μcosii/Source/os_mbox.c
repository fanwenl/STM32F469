...
/*
*********************************************************************************************************
*注意区别于信号量的操作，邮箱的操作在两个任务中，一个任务发送消息，另一个任务接收消息，而信号量的操作是，
*同一个任务中要访问信号量，首先要申请信号量，访问完之后需要释放信号量，申请信号量和发送(释放信号量)在一个
*函数中出现
*********************************************************************************************************
*/
...
void  *OSMboxAccept (OS_EVENT *pevent)
{
    ...
    OS_ENTER_CRITICAL();
    pmsg               = pevent->OSEventPtr;              /*为什么要清邮箱？*/
    /*每次访问完邮箱，需要将其清空，如果有新的邮件会重新将邮件指针放到这里，OSMboxPost()函数中*/
...
*                            邮箱的初始值，一般为空 
* Returns    : != (OS_EVENT *)0  is a pointer to the event control clock (OS_EVENT) associated(关联) with the
*                                created mailbox
...
OS_EVENT  *OSMboxCreate (void *pmsg)
{
    ...
    if (pevent != (OS_EVENT *)0) {
        pevent->OSEventType    = OS_EVENT_TYPE_MBOX; /*事件为消息邮箱标志*/
        pevent->OSEventCnt     = 0u;
        pevent->OSEventPtr     = pmsg;           /* Deposit message in event control block             */
#if OS_EVENT_NAME_EN > 0u
        pevent->OSEventName    = (INT8U *)(void *)"?";
#endif
        OS_EventWaitListInit(pevent);           /*清除任务等待列表*/
    }
    ...
    switch (opt) {
        case OS_DEL_NO_PEND:                               /* Delete mailbox only if no task waiting   */
             if (tasks_waiting == OS_FALSE) {
                 ...
                 pevent->OSEventName = (INT8U *)(void *)"?";
#endif
                 pevent->OSEventType = OS_EVENT_TYPE_UNUSED;/*没有等待任务，将事件块返还给事件空列表*/
   ...
   /*邮箱有消息，将消息指针返回，然后清空邮箱*/
    ...
    switch (OSTCBCur->OSTCBStatPend) {                /* See if we timed-out or aborted                */
        case OS_STAT_PEND_OK:
             pmsg =  OSTCBCur->OSTCBMsg;              /*注意这里是OSTCBCur->OSTCBMsg当前任务，由OSMboxPost()修改*/
            *perr =  OS_ERR_NONE;                     /*接收到的邮件消息*/
             break;
        ...
        default:
             OS_EventTaskRemove(OSTCBCur, pevent);    /*超时时已经将任务就绪，现在将任务等待删除*/
             pmsg = (void *)0;
                   ...
                   void      *pmsg)
{
    ...
    if (pevent->OSEventGrp != 0u) {                   /* See if any task pending on mailbox            */
        /*Ready HPT waiting on event，将pmsg指针写入优先级最高的TCB->OSTCBMsg成员中                    */
        /*等待该邮箱的任务有好多个，但是事件只有一个，所以讲pmsg写入到该任务的TCB中                    */
        /*这个时候pmsg还没有写入到pevent->OSEventPtr中，                                               */
        (void)OS_EventTaskRdy(pevent, pmsg, OS_STAT_MBOX, OS_STAT_PEND_OK);
        OS_EXIT_CRITICAL();
        OS_Sched();                                   /* Find highest priority task ready to run       */
        return (OS_ERR_NONE);                         /*注意return*/
    }
                      ...
                      INT8U      opt)
{
    ...
    OS_ENTER_CRITICAL();
    if (pevent->OSEventGrp != 0u) {                   /* See if any task pending on mailbox            */
      /*下面这个if语句用的也比较好，只有opt&OS_POST_OPT_BROADCAST不为0，其他的度执行else语句，仔细体会&的用法，我会用==，哈哈*/  
      if ((opt & OS_POST_OPT_BROADCAST) != 0x00u) { /* Do we need to post msg to ALL waiting tasks ? */
            while (pevent->OSEventGrp != 0u) {        /* Yes, Post to ALL tasks waiting on mailbox     */
                (void)OS_EventTaskRdy(pevent, pmsg, OS_STAT_MBOX, OS_STAT_PEND_OK);
            }/*将psmg指针放到没到TCB结构体中，这就是广播的过程。觉得上面这函数写的很好，可以实现很多的功能。。*/
                    ...
                    OS_MBOX_DATA  *p_mbox_data)
{
    ...
    pdest                   = &p_mbox_data->OSEventTbl[0]; /*指针的复制方式*/
