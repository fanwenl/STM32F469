...
void  OSInitHookBegin (void)                                    /*在系统初始化函数中调运进行一些额外的初始化*/
{
    INT32U   size;
    OS_STK  *pstk;

                                                                /* Clear exception stack for stack checking.            */
    pstk = &OS_CPU_ExceptStk[0];                                /*获得堆栈的地址(也就是数组的地址)*/
...
OS_STK *OSTaskStkInit (void (*task)(void *p_arg), void *p_arg, OS_STK *ptos, INT16U opt)
{
    ...
    /*理解为什么要+1，假如堆栈开始0xfe，则初始化时指针应该为0xff，压栈时指针先--然后在压栈*/
...
#if OS_TMR_EN > 0u
    OSTmrCtr++;                         /*相当于tick*/
    /*每秒的tick数/每个定时器运行的频率（Rate at which timer management task runs (Hz))定时器的单位时间 */
    if (OSTmrCtr >= (OS_TICKS_PER_SEC / OS_TMR_CFG_TICKS_PER_SEC)) {
        OSTmrCtr = 0;/*OS_TICKS_PER_SEC系统时钟，如果该值为100，则系统tick为10ms*/
        OSTmrSignal();/*OS_TMR_CFG_TICKS_PER_SEC定时器单位，如果定义为10，则定时器实际最小单位为100ms，100/10=10,所以定时器最小单位需要20个tick*/
    }
#endif
}
#endif
