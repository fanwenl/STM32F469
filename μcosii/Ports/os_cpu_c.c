...
void  OSInitHookBegin (void)                                    /*ÔÚÏµÍ³³õÊ¼»¯º¯ÊýÖÐµ÷ÔË½øÐÐÒ»Ð©¶îÍâµÄ³õÊ¼»¯*/
{
    INT32U   size;
    OS_STK  *pstk;

                                                                /* Clear exception stack for stack checking.            */
    pstk = &OS_CPU_ExceptStk[0];                                /*»ñµÃ¶ÑÕ»µÄµØÖ·(Ò²¾ÍÊÇÊý×éµÄµØÖ·)*/
    size = OS_CPU_EXCEPT_STK_SIZE;
    while (size > 0u) {
        size--;
       *pstk++ = (OS_STK)0;
    }
    /*Àí½â¶ÔÆë£¬ÏòÏÂÔö³¤µÄÖ¸Õë£¬ÏÈ»ñµÃ¶ÑÕ»ÆðÊ¼µÄµØÖ·£¬Ò²¾ÍÊÇ³õÊ¼»¯µÄÕ»¶¥Ö¸Õë£¬È»ºó&0xFFFFFFF8£¬µØÖ·±äÐ¡¶ÔÆë             */
...
OS_STK *OSTaskStkInit (void (*task)(void *p_arg), void *p_arg, OS_STK *ptos, INT16U opt)
{
    ...
    /*Àí½âÎªÊ²Ã´Òª+1£¬¼ÙÈç¶ÑÕ»¿ªÊ¼0xfe£¬Ôò³õÊ¼»¯Ê±Ö¸ÕëÓ¦¸ÃÎª0xff£¬Ñ¹Õ»Ê±Ö¸ÕëÏÈ--È»ºóÔÚÑ¹Õ»*/
...
#if OS_TMR_EN > 0u
    OSTmrCtr++;                         /*Ïàµ±ÓÚtick*/
    /*Ã¿ÃëµÄtickÊý/Ã¿¸ö¶¨Ê±Æ÷ÔËÐÐµÄÆµÂÊ£¨Rate at which timer management task runs (Hz))¶¨Ê±Æ÷µÄµ¥Î»Ê±¼ä */
    if (OSTmrCtr >= (OS_TICKS_PER_SEC / OS_TMR_CFG_TICKS_PER_SEC)) {
        OSTmrCtr = 0;/*OS_TICKS_PER_SECÏµÍ³Ê±ÖÓ£¬Èç¹û¸ÃÖµÎª100£¬ÔòÏµÍ³tickÎª10ms*/
        OSTmrSignal();/*OS_TMR_CFG_TICKS_PER_SEC¶¨Ê±Æ÷µ¥Î»£¬Èç¹û¶¨ÒåÎª10£¬Ôò¶¨Ê±Æ÷Êµ¼Ê×îÐ¡µ¥Î»Îª100ms£¬100/10=10,ËùÒÔ¶¨Ê±Æ÷×îÐ¡µ¥Î»ÐèÒª20¸ötick*/
    }
#endif
}
#endif
