 ...
 */
/*长数据的格式、最后一个数据在写入的时候做为第一个数据，然后依次是数据，这个是由DSI写入函数决定的*/
const uint8_t lcdRegData1[]  = {0x80,0x09,0x01,0xFF}; /*该数据的写入的顺序是：0xFF 0x80 0x09 0x01*/
...
/*
  ...
  * 
  * 默认定义的portrait mode模式，下面的data27和data28是对landscape mode模式的定义
  */
...
{
	/*使能CMD2命令模式，设置EXTC = 1*/
  DSI_IO_WriteCmd(0, (uint8_t *)ShortRegData1);  /*0x0000 地址偏移寄存器*/
  ...
  DSI_IO_WriteCmd(0, (uint8_t *)ShortRegData2); /*偏移地址80*/
  ...
  /////////////////////////////////////////////////////////////////////
  /*DC 电源设置														*/
  /* PWR_CTRL4 - 0xC5B0h - 178th parameter - Default 0xA8 */
  /* Set gvdd_en_test                                     */
